...
<img src="C:\Users\Yolo\Desktop\note-master\image\定时器框图.jpg" style="zoom:60%;" />
...
*   `TIM_OCInitStructure.TIM_OCMode`：![](C:\Users\Yolo\Desktop\note-master\image\oc模式.jpg)
*   有效电平的状态通过`TIM_OCInitStructure.TIM_OCPolarity`来配置为高电平或者低电平
*   使用高级定时器一定要调用函数：`TIM_CtrlPWMOutputs(TIM8,ENABLE);	//MOE主输出使能	`
*   其中GPIO的模式配置参考：![](C:\Users\Yolo\Desktop\note-master\image\gpio模式配置.png)
...
*   按数据传送的方式，通讯可分为**串行通讯**和**并行通讯**
    ...
    *   <img src="C:\Users\Yolo\Desktop\note-master\image\通讯.jpg" style="zoom:50%;" />
*   根据数据通讯的方向分为：
    *   **全双工：**可同时收发数据
    *   **半双工：**不可同时收发数据
...
*   以下为一些常见的通信方式：![](C:\Users\Yolo\Desktop\note-master\image\通信方式.jpg)
...
*   框图：![](C:\Users\Yolo\Desktop\note-master\image\usart框图.jpg)
*   USART基本结构：![](C:\Users\Yolo\Desktop\note-master\image\usart基本结构.jpg)
...
*   流控：当2个设备传输数据的速率不匹配导致一个设备接受的数据还没处理就有新的数据传来覆盖旧的数据，这里就会出现丢失数据的现象，这时我们可以通过流控来控制设备是否进行发送数据
    ...
    *   硬件流控：<img src="C:\Users\Yolo\Desktop\note-master\image\硬件流控.jpg" style="zoom:50%;" />
...
*   **IIC时序：**
    ...
    *   <img src="C:\Users\Yolo\Desktop\note-master\image\起始.jpg" style="zoom:50%;" />
    ...
    *   <img src="C:\Users\Yolo\Desktop\note-master\image\字节.jpg" style="zoom:50%;" />
    *   **发送应答：**主机在接收完一个字节之后，在下一个时钟发送一位数据，数据0表示应答，数据1表示非应答
    *   **接收应答：**主机在发送完一个字节之后，在下一个时钟接收一位数据，判断从机是否应答，数据0表示应答，数据1表示非应答（主机在接收之前，需要释放SDA）
    *   <img src="C:\Users\Yolo\Desktop\note-master\image\应答.jpg" style="zoom:50%;" />
    *   **特殊情况：**
...
*   SysTick—系统定时器有 4 个寄存器：（第四个寄存器一般不用）![](C:\Users\Yolo\Desktop\note-master\image\systick.jpg)
...
对于CrortexM3/4系列内核中的寄存器都有16个寄存器，寄存器组通常都是CPU用于数据处理和运行控制的：![](C:\Users\Yolo\Desktop\note-master\image\cortex 么.png)
...
* 假如mian函数的栈为：
  ...
  | 0x118   | ebp指向栈底，也就是最高的地址 |
  | ------- | ----------------------------- |
  | 0x114   | data                          |
  | ....... | ......                        |
  | 0x108   | esp指向栈顶，也就是最低的地址 |

* 调用子函数fun时往往有下面几个步骤：

  | 汇编指令      | 指令归属函数 | SP变化 | 作用                                                         |
  | ------------- | ------------ | ------ | ------------------------------------------------------------ |
  | push arg2     | 主函数       | sp-4   | 压入函数参数，从最后的参数开始                               |
  | push arg1     | 主函数       | sp-4   | 压入函数参数                                                 |
  | call fun      | 主函数       | sp-4   | 开始调用子函数，同时保存返回地址                             |
  | push ebp      | 子函数       | sp-4   | 压入上一个函数的基地址，并更新本函数的基址                   |
  | push ebp，esp | 子函数       | sp-4   | 将当前sp存入bp，保存函数的栈底地址,上一个函数的esp就是当前函数的ebp |
  | sub sp，#num  | 子函数       | sp-num | 为子函数分配栈空间                                           |
  | ......        | 子函数       | ...... | 函数的具体实现步骤                                           |
  | pop ebp       | 子函数       | sp+4   |                                                              |
  | ret           | 子函数       | sp+4   |                                                              |
...
* 因为刚开始有 push ebp 的操作，在调用函数的末尾，也需要 pop ebp。ret 操作，会将栈中的放回地址弹出到 eip，sp 同时加 4。这样被调函数执行完毕，下一步将继续执行被调用函数之前的指令。但是此时，sp 指向原先的 arg1，并没有指向原先主函数的栈顶。如果原先栈中还有其他数据，sp 没有归位会导致主函数引用栈中数据出错，所以需要堆栈平衡，比如 **cdecl** 方式和 **stdcall** 方式，cdecl 方式中，由主程序执行 `add esp, n` 指令调整 esp，达到堆栈平衡。在 stdcall 方式中，由子程序在返回时，执行 `ret n` 平衡堆栈。n 其实就是函数的参数所占的空间大小
...
```c
![堆栈使用](C:\Users\Yolo\Desktop\note-master\image\堆栈使用.png)/* stack_test.c */
#include <stdio.h>
...
![](C:\Users\Yolo\Desktop\note-master\image\堆栈使用.png)

![](C:\Users\Yolo\Desktop\note-master\image\堆栈使用二.png)

* push 0x2：将形参变量2压入栈中，从后面的形参开始
* push 0x1：将形参变量1压入栈中
* call 1189<func>：调用地址为1189的子函数func，将返回地址压入栈中，esp = esp-4
* ![](C:\Users\Yolo\Desktop\note-master\image\堆栈使用三.png)
* 之后进入子函数
* push ebp：将ebp寄存器压入栈中，作为自己的ebp，这里可以通过ebp来获取传入形参的值，arg1=ebp+0x08，arg2=ebp+0x0C，0x04是返回地址
* mov ebp，esp：将esp的值传入到ebp中，这时ebp和esp占用同一个地址
* sub esp，0x10：esp下移0x10的大小，为自己分配栈空间
* 之后就是子函数的实现过程
* 执行完之后将自己的栈空间全部释放，最后pop ebp，ret
* pop ebp：基地址出栈，esp+4
* ret：相当于**pop eip; esp = esp + 4**，将返回地址存入eip中，而eip寄存器存放的是下一条指令的地址
...
## 1.General Description(简要说明)

![](C:\Users\Yolo\Desktop\note-master\image\max96717.png)

* 图像传感器采集数据通过MIPI CSI-2(Camera)接口标准，在物理层面，MIPI CSI-2一般会有1对IIC通信引脚，1对MIPI差分时钟引脚，1-4对MIPI差分数据信号引脚[MIPI CSI-2](https://mp.weixin.qq.com/s?__biz=MzI4NjE3MzUwMA==&mid=2652139052&idx=1&sn=21c006f96d3bfd363be5c32829714d28&chksm=f0003e7fc777b769c5e1f49a8e4c80597f2a4d8c4e021398847abdda6e193e8922a367e253cf&token=334993533&lang=zh_CN)![](C:\Users\Yolo\Desktop\note-master\image\MIPI-CSI2.png)

* 然后连接到MAX9295A或者MAX96717F这种加串芯片将并行数据转化成串行数据流，然后通过更高的频率以GMSL协议进行传输从而解决并行总线通信中传输大量数据的时候成本，重量以及EMI电磁干扰的问题
* 之后将数据传输到解串器MAX9296A中进行数据解串成并行数据通过MIPI CSI-2接口传输到FPGA/ARM主控芯片进行处理，在显示到LCD上



**应用：**

* 高级驾驶员辅助系统(ADAS)，自动刹车，自适应巡航控制

* 激光雷达(LiDAR)系统
* 雷达(RADAR)系统
* 4Mp 40fps的前向摄像头系统(FVC)
* 环绕音响系统(SVS)
* 驾驶员监控系统(DMS)
* 后视摄像机(RVC)
* 具有多个同步摄像头的系统
...
* 四通道D-PHY MIPI CSI-2 v1.3输入端口
* 高级MIPI D-PHY v1.2接收器，额定值为2.5Gbps
  * 极性翻转和数据通道重新分配
* 在单线上的全双工功能
  * 3Gbps前向链路速率
  * 187.5Mbps的反向链路速率
* CSI-2 ECC，校验和本地，远程错误检测和标记
* 符合ASIL-B
  * ASIL是汽车安全完整性等级，ASIL-B只是其中的一个等级
  * [ASIL汽车安全完整性等级](https://zhuanlan.zhihu.com/p/36243044)
  * 隧道模式下通过CRC实现端到端的数据完整性
  * 使用R-S FEC(前向纠错编码)来保护前向视频和控制信道数据
  * 使用CRC保护旁道和视频数据
* 持续的链路余量监控和优化
  * 反向信道自适应均衡
  * 用于连续链路裕度诊断的反向通道开眼监视器
* 用于设备配置和外围设备通信的并行旁路通道
  * 使用IIC/UART，SPI，寄存器可编程的GPIO
* 为图像传感器生成参考时钟
* 模具温度监测器、内部和外部电源电压监测器
* 紧凑型5mm x 5mm TQFN侧面可湿封装，间距为0.5mm
...
* **SIOP：**作为同轴/双绞线串行数据输入输出的差分信号线正端(GMSL2)
* **SION：**作为同轴/双绞线串行数据输入输出的差分信号线负端(GMSL2)
* **D2P：**
  * D2P(默认)：作为MIPI CSI-2接口中Lane2的正端输入
  * D0：作为并行接口D0的数据输入
* **D2N：**
  * D2N(默认)：作为MIPI CSI-2接口中Lane2的负端输入
  * D1：作为并行接口D1的数据输入
* **D3P：**
  * D3P(默认)：作为MIPI CSI-2接口中Lane3的正端输入
  * D4：作为并行接口D4的数据输入
* **D3N：**
  * D3N(默认)：作为MIPI CSI-2接口中Lane3的负端输入
  * D5：作为并行接口D5的数据输入
* **D2P：**
  * D2P(默认)：作为MIPI CSI-2接口中Lane2的正端输入
  * D0：作为并行接口D0的数据输入
* **D2N：**
  ...
  * D1：作为并行接口D1的数据输入
* **D0P：**
  * D0P(默认)：作为MIPI CSI-2 D-PHY接口中Lane0的正端输入
  * D6：作为并行接口D6的数据输入
  * D0A：作为MIPI CSI-2 C-PHY接口中Lane0的A输入
* **D0N：**
  * D0N(默认)：作为MIPI CSI-2 D-PHY接口中Lane0的负端输入
  * D7：作为并行接口D7的数据输入
  * D0B：作为MIPI CSI-2 C-PHY接口中Lane0的B输入
* **CKP：**
  * CKP(默认)：作为MIPI CSI-2 D-PHY接口中Clock lane的正端输入
  * D8：作为并行接口D6的数据输入
  * D0C：作为MIPI CSI-2 C-PHY接口中Lane0的C输入
* **CKN：**
  * CKN(默认)：作为MIPI CSI-2 D-PHY接口中Clock lane的负端输入
  * D9：作为并行接口D9的数据输入
  * D1A：作为MIPI CSI-2 C-PHY接口中Lane1的A输入
* **D1P：**
  * D1P(默认)：作为MIPI CSI-2 D-PHY接口中Lane1的正端输入
  * D10：作为并行接口D10的数据输入
  * D1B：作为MIPI CSI-2 C-PHY接口中Lane1的B输入
* **D1N：**
  * D1N(默认)：作为MIPI CSI-2 D-PHY接口中Lane1的负端输入
  * D11：作为并行接口D11的数据输入
  * D1C：作为MIPI CSI-2 C-PHY接口中Lane1的C输入
* **MFP0：**
  * GPIO0(默认)：可配置的多功能输入或者输出，上电默认用一个1M的电阻下拉至高阻态
  * SCLK：SPI Clock，当配置成SPI主机，作为推挽时钟输出。当配置成从机，作为带1M下拉电阻(到地)的时钟输入
  * PCLK：并行接口像素时钟输入
* **MFP1/CFG0：**
  * GPO1(默认)：可配置的多功能输入或者输出，上电默认用一个1M的电阻下拉至高阻态
  * CFG0：可配置的引脚，通过一个电阻分压器连接在 $V_{DDIO}$与$ground$之间，根据分压值来设置通信方式和设备地址
  * SS1：当配置成SPI的主机时，作为从机选择输出
  * BNE：当配置成SPI的从机时，BNE输出表明SPI数据是有效的
* **MFP2/CFG1：**
  * GPO2(默认)：可配置的多功能输入或者输出，上电默认用一个1M的电阻下拉至高阻态
  * CFG1：可配置的引脚，通过一个电阻分压器连接在 $V_{DDIO}$与$ground$之间，根据分压值来设置传输线是同轴电缆还是STP电缆
  * RCLKOUT(Alternate)：25MHz的参考频率输出，可以2分频或者4分频
  * DPLL_OUT(Alternate)：数字锁相环输出，可编程为1-75MHz


> CFG0和CFG1的电压水平决定设备地址，IIC或者UART控制通道，GMSL3或者GMSL2协议，串行速率，同轴电缆还是STP电缆

* **MFP3：**
  * GPIO3(默认)：可配置的多功能输入或者输出，上电默认用一个1M的电阻下拉至高阻态
  * ADC0：支持电压监控
  * LOCK：高电平表示 PLL 已锁定并具有正确的串行字边界对齐
  * ERRB：线路故障或错误输出， 当检测到 LMN0 或 LMN1 线路故障或检测到 CSI-2 ECC 或校验和错误时变低
  * VS：并行接口垂直同步输入

* **MFP4：**
  * GPIO4(默认)：可配置的多功能输入或者输出，上电默认用一个1M的电阻下拉至高阻态
  * RCLKOUT：25MH的参考频率输出，可2分频或者4分频
  * DPLL_OUT：数字PLL输出，也通过编程配置成1MHz到75MHz
  * SS2：当配置为SPI主机，作为从机选择输出
  * RO：当配置成SPI的从机时，SPI主机能够在启动与远程从机的数据传输时从本地SPI从机读取，RO作为输入端口(相当于中间的交换机)
  * HS：并行接口水平同步输入

* **MFP5：**
  * GPI5/ODO5(默认)：配置成多功能输入或者开漏输出，上电默认用一个1M的电阻下拉至高阻态
  * ADC1：电源电压模拟监控输入
  * LMN0：线路故障监控输入
  * D2：并行接口数据输入

* **MFP6：**
  * GPI6/ODO6(默认)：配置成多功能输入或者开漏输出，上电默认用一个1M的电阻下拉至高阻态
  * ADC2：电源电压模拟监控输入
  * LMN1：线路故障监控输入
  * D3：并行接口数据输入

* **MFP7：**
  * GPI7(默认)：可配置成多功能输入，上电默认是用1M电阻下拉
  * GPIO7：配置成多功能输入或者输出
  * MOSI：SPI主机输出，驱动数据到一个额外的SPI从机，当配置为主机时，使用推挽输出传输数据到从机，当配置为从机，输入带有内部1M下拉至地的电阻，可从外部主机接收数据
  * SDA1：作为直连IIC的数据输入和输出
  * RX1：作为直连串口的输入
  * LOCK(Alternate)：高电平表示 PLL 已锁定并具有正确的串行字边界对齐
  * D12：并行接口的数据输入

* **MFP8：**
  * GPIO8(默认)：可配置为多功能输入或者输出，带有内部40K的上拉电阻连接到$V_{DDIO}$,上电默认是高电平的GPIO
  * MS：模式选择输入，为控制通道选择基本模式或者旁路模式
  * MISO：输入，从外部SPI从机设备接收数据
  * SCL1：直连的IIC时钟输入，配置为开漏输出带内部40k上拉电阻到$V_{DDIO}$
  * TX1：作为直连UART的输出口带有内部40k上拉电阻到$V_{DDIO}$
  * ERRB(Alternate)：线路故障或错误输出， 当检测到 LMN0 或 LMN1 线路故障或检测到 CSI-2 ECC 或校验和错误时变低
  * D13：并行接口的数据输入

* **MFP9：**
  * SDA_RX(默认)：IIC串行数据输入输出或者UART数据接收口，带有内部上拉40k电阻到$V_{DDIO}$（SDA或者RX通过CFG0在上电时的状态来选择）
  * GPI9/ODO9：多功能输入或者开漏输出
  * SDA2_RX2：直连IIC串行数据输入输出或者UART2接收口，内部带40k电阻上拉

* **MFP10：**
  * SCL_TX(默认)：IIC串行时钟或者UART输出口，内部带40k电阻上拉到$V_{DDIO}$
  * ODO10_GPI10：开漏输出或者多功能输入
  * SCL2_TX2：直连IIC串行时钟输入输出或者UART2输出口，带有内部上拉电阻到$V_{DDIO}$
...
<img src="C:\Users\Yolo\Desktop\note-master\image\clip_image002.jpg" style="zoom:100%;" />
...
![](C:\Users\Yolo\Desktop\note-master\image\clip_image002-1547547642923.jpg)
...
![](C:\Users\Yolo\Desktop\note-master\image\clip_image002-1547547896341.jpg)
...
![](C:\Users\Yolo\Desktop\note-master\image\clip_image002-1547604555425.jpg)
...
![](C:\Users\Yolo\Desktop\note-master\image\clip_image002-1547606475892.jpg)
...
![](C:\Users\Yolo\Desktop\note-master\image\clip_image002-1547608564071.jpg)
...
![](C:\Users\Yolo\Desktop\note-master\image\v2-bc0091ec15f7bd19094e890bf3e90418_1440w.gif)

#### 1.2 驱动电路实现

逆变电路：

![](C:\Users\Yolo\Desktop\note-master\image\v2-de99d63cac7edac6868bd829ce92685f_1440w.jpg)
...
![](C:\Users\Yolo\Desktop\note-master\image\v2-b12a2650b001faa9cd31ecbb309eaca0_1440w.webp)
...
​			![](C:\Users\Yolo\Desktop\note-master\image\v2-b6165e60926bea9399b794a736d5e335_1440w.jpg)
...
* **Clack变换：**
  ...
  <img src="C:\Users\Yolo\Desktop\note-master\image\v2-a105f3cbf5dfc208c829c7364257654a_1440w.webp" style="zoom:80%;" />

​													![](C:\Users\Yolo\Desktop\note-master\image\clack.jpg)  

**在右边的矩阵上应该还要乘以$\frac{\sqrt2}{\sqrt3}$以保证幅值相等**

将$I_a,I_b,I_c$按直角坐标系分解为 $I_\alpha$,$I_\beta$，控制电机的波形图变成下方的image

![](C:\Users\Yolo\Desktop\note-master\image\v2-ddb74b212baee03b36eecb32ec54e420_1440w.webp)
...
>
> <img src="C:\Users\Yolo\Desktop\note-master\image\欧拉clack.jpg" style="zoom:50%;" />
>
> 比较幅值发现是原矢量的$\frac{3}{2}$，所以在矩阵的右边乘以$\frac{2}{3}$来保证幅值相等
>
> <img src="C:\Users\Yolo\Desktop\note-master\image\欧拉clack2.jpg" style="zoom:50%;" />
>
...
<img src="C:\Users\Yolo\Desktop\note-master\image\v2-d15d057327992a5c50016aea5bb7201b_1440w.webp" style="zoom:50%;" />

​														<img src="C:\Users\Yolo\Desktop\note-master\image\park.jpg" style="zoom:80%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\v2-1918346a89639774454b7136889d3816_r.jpg" style="zoom:50%;" />

可知：矩阵相乘的结果是常量

![](C:\Users\Yolo\Desktop\note-master\image\v2-d6116ce165ec51df51469ad6ddd4d134_720w.webp)
...
*  **电流环**：
  ...
  <img src="C:\Users\Yolo\Desktop\note-master\image\v2-92e0d6eaa15bff745a4cd024362f53eb_720w.webp" style="zoom: 80%;" />
...
$I_q和$I_d$相当于将转子的磁链进行了解耦，分解为了转子旋转的**径向**和**切向**这两个方向的变量：

* 其中$I_q$是我们需要的，代表了期望的力矩输出
* 而$I_d$是我们不需要的，我们希望尽可能把它控制为0

![](C:\Users\Yolo\Desktop\note-master\image\v2-1926583e289cf44fe24f4a72944e2a08_720w.webp)
...
<img src="C:\Users\Yolo\Desktop\note-master\image\v2-2fc3a67057c1f08b4c5969551d417003_720w.webp" style="zoom:80%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\v2-3ec15f2e8ab0b97b7818e1d94d9dbd9b_720w.webp" style="zoom:80%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\v2-af17ac8465092f198e2f4f2ea8dad752_720w.webp" style="zoom:80%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\v2-28c5ce7876603e45d61668e290e371e3_720w.jpg" style="zoom:80%;" />

此时等效电路如图：

<img src="C:\Users\Yolo\Desktop\note-master\image\v2-f4cd3936b0c1599086576e4ba5f7f9bd_720w.jpg" style="zoom:80%;" />

因此电机中三个**相电压**（相电压是每相相对于电机中间连接点的电压）可以表示为：

![](C:\Users\Yolo\Desktop\note-master\image\三相.jpg)

$U_{dc}$是电源电压，再合成矢量：

<img src="C:\Users\Yolo\Desktop\note-master\image\矢量合成.jpg" style="zoom:80%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\v2-1d0392a19bc58c20f2c440d9c06edc8c_720w.webp" style="zoom:80%;" />

> 注意上图中的(100)矢量方向和AO方向是相反的（变成OA方向），这跟正方向的定义有关，这样的规定更直观一些。
> 同时可以注意到两个零矢量其实和原点重合了，因为这两个状态下电机中产生力矩的磁场为0（不考虑旋转过程中的反电动势产生的阻力力矩）。

<img src="C:\Users\Yolo\Desktop\note-master\image\矢量计算.jpg" style="zoom:50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\v2-a0233fb57d155f8979997003d9afcfa6_720w.webp" style="zoom:50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\v2-47aaa9843d2594f1ffd82eed2727d7d3_1440w.webp" style="zoom: 50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\电压矢量合成.webp" style="zoom: 80%;" />

> 乘以$\frac{2}{3}$是为了使幅值统一

变换到$\alpha和\beta$坐标系下如下图：

<img src="C:\Users\Yolo\Desktop\note-master\image\α-β坐标系.webp" style="zoom: 50%;" />

***

> 当我们在三相上加的是相差120°的正弦波电压时，如图：
>
> <img src="C:\Users\Yolo\Desktop\note-master\image\加正弦波电压.webp" style="zoom:50%;" />
>
> $u_m$是他们的幅值，$w_e$是他们的角频率都相等只有相位相差120°
>
> 将他们合成：
>
> <img src="C:\Users\Yolo\Desktop\note-master\image\正弦波合成.webp" style="zoom:80%;" />
>
> 可知合成的电压是幅值不变，相位随着角频率改变而改变，如图：
>
> <img src="C:\Users\Yolo\Desktop\note-master\image\电压矢量合成动图.gif" style="zoom:67%;" />
>
...
<img src="C:\Users\Yolo\Desktop\note-master\image\扇区分解.webp" style="zoom:50%;" />



<img src="C:\Users\Yolo\Desktop\note-master\image\SharedScreenshot.jpg" style="zoom:50%;" />

从而得到：

<img src="C:\Users\Yolo\Desktop\note-master\image\SharedScreenshot2.jpg" style="zoom:50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\开关顺序.webp" style="zoom:50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\SharedScreenshot3.jpg" style="zoom:50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\扇区判断.webp" style="zoom:50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\扇区表.webp" style="zoom:50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\扇区基向量顺序.webp" style="zoom: 50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\SharedScreenshot4.jpg" style="zoom:50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\扇区周期表.webp" style="zoom:50%;" />

<img src="C:\Users\Yolo\Desktop\note-master\image\SharedScreenshot5.jpg" style="zoom:50%;" />
...
<img src="C:\Users\Yolo\Desktop\note-master\image\基向量切换时间点.webp" style="zoom: 50%;" />

$a$表示$a$相电压的导通和关闭，设 $T_{cm1},T_{cm2},T_{cm3}$是 $a，b，c$相上关闭到导通的时间，对任意一个参考电压：

<img src="C:\Users\Yolo\Desktop\note-master\image\基向量电压三相切换时间点.webp" style="zoom:50%;" />
...
*   `TIM_BDTRInitTypeDef`**（配置断路和死区结构体）**：
    ...
    *   <img src="C:\Users\Yolo\Desktop\note-master\image\高级定时器框图.jpg" style="zoom:67%;" />
...
*   死区时间计算：
    ...
    <img src="C:\Users\Yolo\Desktop\note-master\image\死区时间计算.jpg" style="zoom:50%;" />
...
*   定点数格式：小数点的位置是固定的，int,short,long都是整数，所以将小数点的位置设在最后，也就是没有小数，只表示整数：![](C:\Users\Yolo\Desktop\note-master\image\定点数格式.jpg)
...
*   **浮点数格式：**![](C:\Users\Yolo\Desktop\note-master\image\浮点数模式.jpg)
